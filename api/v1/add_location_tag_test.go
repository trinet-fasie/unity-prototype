...
func TestAddLocationTagApi(t *testing.T) {
	Convey("Given invalid request", t, func() {
		ts, _ := GetTestHelpers(t)

		Convey("When client POST /v1/add-location-tag", func() {
			res, err := http.Post(ts.URL+"/v1/add-location-tag", "application/json", strings.NewReader(`{}`))
				...
				So(res, ShouldBeValidBadRequestJsonResponse, `
{
  "Status": "fail",
  "Data": {
    "AddLocationTagDto.Text": {
      "ActualTag": "required",
      "Field": "Text",
      "FieldNamespace": "AddLocationTagDto.Text",
      "Kind": 24,
      "Name": "Text",
      "NameNamespace": "Text",
      "Param": "",
...
`)
		...
		Convey("When client POST /v1/add-location-tag with too long tag text", func() {
			res, err := http.Post(ts.URL+"/v1/add-location-tag", "application/json", strings.NewReader(`{
...
`)
	...
	Convey("Given valid request with non-existent text in database", t, func() {
		...
		sqlMocker.ExpectQuery(`SELECT.*FROM\s+location_tags`).WithArgs("some valid tag").WillReturnError(sql.ErrNoRows)

		rows := sqlmock.NewRows([]string{"id", "created_at", "updated_at"}).
			AddRow(1, "2018-01-01 00:00:00", "2018-01-02 00:00:00")
		sqlMocker.ExpectQuery(`INSERT\s+INTO\s+location_tags`).WithArgs("some valid tag").WillReturnRows(rows)
...
`)
	...
	Convey("Given valid request with existent text in database", t, func() {
		...
		sqlMocker.ExpectQuery(`SELECT.*FROM\s+location_tags`).WithArgs("some valid tag").WillReturnRows(rows)
